"""
EURAUD 0.0006 — R031 KAMA Ribbon 5/13/30 + Trend-Regime Gate Sweep

Follow-up to the R030 dead-end sweep. Locks the best R030 ribbon (5/13/30)
and adds a single low-overlap trend-regime gate (ADX / CHOP / BB_BW / STREAK),
testing each at 3 thresholds.

Discipline: same 3-way split as the prior sweep, same locked HOLDOUT rules.
HOLDOUT is now used for a SECOND time on this instrument — per R22 extension,
if a config passes here, treat it as a candidate for fresh-data revalidation
before deploy, not a deploy signal.
...
Compute budget
--------------
  5 regimes × 3 thresholds = 15 combos
  (of which "none" × {0,1,2} are equivalent — 2 duplicates will be no-ops)
  15 × 3 windows = 45 backtests
  Wallclock: <30 seconds on 20 workers.

Usage
-----
  python renko/euraud_kama_ribbon_gated_sweep.py
"""
...
import r031_kama_ribbon_gated as strat
...
OUTPUT_FILE = ROOT / "ai_context" / "euraud_kama_ribbon_gated_results.json"
...
def gate_label(p):
    """Human-readable threshold value for display."""
    if p["regime"] == "none":
        return "none"
    t = strat.THRESH_TABLE[p["regime"]][p["thresh"]]
    return f"{p['regime']}>={t}" if p["regime"] in ("adx", "bb_bw", "streak") else f"{p['regime']}<={t}"
...
def main():
    ...
    # Dedupe: "none" × threshold 0/1/2 are identical — keep only threshold 0
    seen = set()
    dedup = []
    for c in combos:
        key = ("none", 0) if c["regime"] == "none" else (c["regime"], c["thresh"])
        if key in seen:
            continue
        seen.add(key)
        dedup.append(c)
    ...
    print("=" * 70)
    print("EURAUD 0.0006 — R031 KAMA Ribbon 5/13/30 + Regime Gate Sweep")
    print("=" * 70)
    print(f"Strategy   : r031_kama_ribbon_gated (ribbon LOCKED 5/13/30)")
        ...
        for fut in as_completed(futures):
            ...
            if done % 10 == 0 or done == len(tasks):
                print(f"  [{done:>3}/{len(tasks)}] {r['window']:<8} "
                      f"{fmt_row(r)} | {gate_label(r['params'])}")

    # Pivot
    by_combo = {}
    for r in results:
        key = (r["params"]["regime"], r["params"]["thresh"])
        by_combo.setdefault(key, {})[r["window"]] = r

    # ── Print per-combo 3-window table ───────────────────────────────────────
    print()
    print("=" * 70)
    print("PER-COMBO SUMMARY (all windows)")
    print("=" * 70)
    order = [(c["regime"], c["thresh"]) for c in combos]
    for key in order:
        wmap = by_combo.get(key, {})
        label_p = {"regime": key[0], "thresh": key[1]}
        print(f"\n  {gate_label(label_p)}")
    ...
    for key, wmap in by_combo.items():
        ...
        if val["net"] <= 0:
            continue
        finalists.append({
            "params":   {"regime": key[0], "thresh": key[1]},
            "train":    train,
            "validate": val,
            "wr_delta": wr_delta,
        })
    ...
    if not top_finalists:
        ...
        for i, f in enumerate(top_finalists, 1):
            key = (f["params"]["regime"], f["params"]["thresh"])
            hold = by_combo[key].get("HOLDOUT")
            f["holdout"] = hold
            if not hold:
                f["passes"] = False
                print(f"  [{i}] NO HOLDOUT DATA  {gate_label(f['params'])}")
                continue
            passes = (
            ...
            print(f"  [{i}] {tag}  {gate_label(f['params'])}")
    ...
    with open(OUTPUT_FILE, "w") as f:
        json.dump({
            "strategy":   "r031_kama_ribbon_gated",
            ...
            "thresh_table":   strat.THRESH_TABLE,
