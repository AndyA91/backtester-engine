#!/usr/bin/env python3
"""
mym_sweep_v3.py -- MYM Phase 3: Push n_bricks + ADX ceiling

Phase 2 findings:
  - n_bricks=9 dominated ALL bricks (grid ceiling!) -> expand 9-13
  - ADX>=40 best for bricks 12-14 -> push to 35-50
  - cooldown=35-40 best -> expand 30-50
  - psar_dir/mk_regime/ema_cross top P6 gates, escgo_cross weak (except B15)
  - sto_tso best oscillator

Changes vs v2:
  - n_bricks: [9, 10, 11, 12, 13]   (find the ceiling)
  - cooldown: [30, 35, 40, 45, 50]   (higher range)
  - ADX: [35, 40, 45, 50]            (push higher)
  - P6 gates: keep top 4 + none
  - Osc: [none, sto_tso, macd_lc]
...
# ---- Instrument configs (same as v1/v2) --------------------------------------
...
PARAM_GRID = {
    "n_bricks": [9, 10, 11, 12, 13],
    "cooldown": [30, 35, 40, 45, 50],
}

ADX_THRESHOLDS = [35, 40, 45, 50]
P6_GATES = ["none", "psar_dir", "mk_regime", "ema_cross", "escgo_cross"]
...
# ---- Gate pre-computation (same as v2) ---------------------------------------
...
def _summarize(all_results):
    ...
    for inst in sorted(INSTRUMENTS.keys()):
        ...
        print(f"\n{'='*90}")
        print(f"  {cfg['label']}  (Phase 3 -- High n_bricks + ADX)")
        print(f"{'='*90}")
    ...
    print(f"{'='*90}")
    try:
        with open(ROOT / "ai_context" / "mym_sweep_v2_results.json") as f:
            v2_data = json.load(f)
        for brick in [11, 12, 13, 14, 15]:
            v2_best = [r for r in v2_data if r["brick"] == brick
                       and r["oos_trades"] >= 10 and not math.isinf(r["oos_pf"])]
            v3_best = [r for r in all_results if r["brick"] == brick
                       and r["oos_trades"] >= 10 and not math.isinf(r["oos_pf"])]
            ...
            if v2_top and v3_top:
                delta = (v3_top["oos_pf"] - v2_top["oos_pf"]) / v2_top["oos_pf"] * 100
                print(f"  Brick {brick}: v2={v2_top['oos_pf']:>7.2f}  v3={v3_top['oos_pf']:>7.2f}  "
                      f"delta={delta:>+6.1f}%  "
                      f"| {v3_top['stack']} n={v3_top['n_bricks']} cd={v3_top['cooldown']}")
...
def main():
    ...
    out_path = ROOT / "ai_context" / "mym_sweep_v3_results.json"
    ...
    print("MYM Phase 3 Sweep -- High n_bricks + ADX")
