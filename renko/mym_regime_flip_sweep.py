"""
MYM Brick 30 — MYM003 Regime Flip Sweep
========================================

Cross-instrument carryover of GJ001 to MYM Renko 30. Per R20, re-optimize
on MYM data — do NOT import GBPJPY params.

Entry: ER regime flips RANGE → TREND + brick direction.
Exit:  First opposing brick (R1).
TP:    Optional bracket in bricks (0 = disabled).

MYM conventions (R24):
  qty_value=0.5 ($0.50/point multiplier workaround)
  commission_pct=0.004 (~$1.88 RT, close to $1.90 actual)
...
Sweep grid (~660 combos × 3 windows = ~1,980 backtests)
--------------------------------------------------------
  ...
  persistence:   [0, 2]                    = 2
  tp_bricks:     [0, 3, 5, 8]              = 4
  (minus invalid: trend_thresh <= range_thresh)

No trail_bricks sweep — GJ001 confirmed trail>1 is dead (R1 holds).
...
Usage
-----
  python renko/mym_regime_flip_sweep.py
"""
...
OUTPUT_FILE = ROOT / "ai_context" / "mym_regime_flip_results.json"
...
def make_cfg(start, end):
    return BacktestConfig(
        ...
        slippage_ticks=0,
        qty_type="fixed",
        qty_value=0.5,  # R24: $0.50/point multiplier workaround
        pyramiding=1,
        start_date=start,
        end_date=end,
        take_profit_pct=0.0,
...
def label(p):
    return (f"er{p['er_lookback']}s{p['er_smooth']} "
            ...
            f"p{p['persistence']} tp{p['tp_bricks']}b")
...
def evaluate(results):
    ...
    print()
    print("=" * 78)
    print(f"MYM Brick 30  —  {len(finalists)} finalists passed TRAIN→VAL filters")
    print("=" * 78)
...
def main():
    ...
    print("=" * 78)
    print("MYM Brick 30 — MYM003 Regime Flip Sweep")
    print("=" * 78)
    print(f"Strategy   : mym003_regime_flip (ER regime flip + brick direction)")
