"""
5-min Showdown v1 — Same 5 architectures as multi_indicator_eurusd_1.py
but run on 5-minute data for both EURUSD and GBPJPY.

Robustness check: do the 3-min winners (VWAP Bands, RSI Reversal)
hold up on a different timeframe?
"""
...
# =============================================================================
# Strategy builders (identical logic to multi_indicator_eurusd_1.py)
# =============================================================================
...
def strat_rsi(df, per=14, os_=25, ob=75, cd=60, use_vwap=False):
    c = df["Close"].values; n = len(c)
...
def main():
    TS = "2026-02-17"
    rp = Path(__file__).resolve().parent / "showdown_5min_results.txt"
    out = open(rp, "w", encoding="utf-8")
    def log(m=""):
        print(m); out.write(m + "\n"); out.flush()

    log("5-MIN SHOWDOWN v1 — 5 architectures x 2 pairs")
    ...
    for df_file, pair, qty in [
        ...
        if best:
            ...
            log(f"     Band={best['bm']}sig CD={best['cd']} RSI={best['rf']} TP={best['tp']} SL={best['sl']}")
            log(f"     Full: PF={best['pf']:.3f} WR={best['wr']:.1f}% Tr={best['tr']} Net={best['net']:+.2f} DD={best['dd']:.2f}%")
            log(f"     OOS:  PF={oos['pf']:.3f} WR={oos['wr']:.1f}% Tr={oos['tr']} DD={oos['dd']:.2f}%")
    ...
    out.close()
    print(f"\n  Done! -> strategies/showdown_5min_results.txt")
