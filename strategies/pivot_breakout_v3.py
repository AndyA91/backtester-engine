"""
Pivot Breakout v3 optimizer.

Fork of pivot_breakout_v2.py with expanded sweep dimensions:
- re-opened pivot_len and adx_threshold
- adjusted ER range lower
- dropped HTF Supertrend from the sweep

Sweep size: 72 combinations.
"""
...
def main() -> None:
    ...
    out_path = here / "pivot_breakout_v3_results.txt"
    ...
    cooldown_vals = [90]
    session_vals = [True]
    pivot_len_vals = [5, 8, 13]
    adx_threshold_vals = [25, 30]
    kama_slope_vals = [False]
    er_threshold_vals = [0.0, 0.10, 0.15, 0.20]
    ...
    if len(all_params) != 72:
        raise RuntimeError(f"Expected 72 combinations, got {len(all_params)}")
    ...
    sanity_result = _worker((90, True, 8, 30, False, 0.0, 0))
    print(
    ...
    with out_path.open("w", encoding="utf-8") as f:
        f.write("Pivot Breakout v3 Optimization Results\n")
        f.write("Goal: Improve breakout quality via ER + BB squeeze while re-opening pivot_len and adx_threshold\n")
        f.write(
        ...
        f.write(
            "Sweep: pivot_len in [5, 8, 13], adx_threshold in [25, 30], er_threshold in [0.0, 0.10, 0.15, 0.20], bb_squeeze_lookback in [0, 10, 20]\n\n"
        )
