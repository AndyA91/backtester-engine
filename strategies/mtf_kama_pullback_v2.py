"""
MTF KAMA Pullback v2 optimizer.

Built from validated v4 baseline with dual entry modes:
- Mode A: v4 momentum entries (ADX + slope1_min)
- Mode B: anchor-KAMA pullback entries (failed retest with entry-side anchor confirmation)
...
def _build_signals(
    ...
    for i in range(1, n):
        ...
        if use_addon:
            ...
            modeB_long = tf1_bull and tf2_bull and cross_up_i and in_session and recently_below_anchor and (close[i] > kama_anchor[i])
            modeB_short = tf1_bear and tf2_bear and cross_dn_i and in_session and recently_above_anchor and (close[i] < kama_anchor[i])
...
def main() -> None:
    ...
    out_path = here / "mtf_kama_pullback_v2_results.txt"
    ...
    with out_path.open("w", encoding="utf-8") as f:
        f.write("MTF KAMA Pullback v2 Optimization Results\n")
        ...
        f.write("- v1 top-3 (for comparison):\n")
        f.write("  1) PF=1.7334, Net=12.48, Trades=53, anchor_len=150, touch_lookback=50\n")
        f.write("  2) PF=1.6791, Net=10.20, Trades=43, anchor_len=150, touch_lookback=10\n")
        f.write("  3) PF=1.6777, Net=11.09, Trades=50, anchor_len=200, touch_lookback=50\n")
        if zero_trade:
