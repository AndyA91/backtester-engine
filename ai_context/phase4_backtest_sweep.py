"""Phase 4 backtest sweep runner — EA015 + EA016 (new BC L1 creative strategies).

New strategies:
  EA015 — BC L1 Swing Trade Oscillator mean-reversion entries (EURAUD)
  EA016 — BC L1 Momentum Crossover Pro + Dynamic Defense Line (EURAUD)

Benchmark: EA008 OOS PF 10.62

Outputs:
  ai_context/ea_phase4_creative_results.json
"""
...
CREATIVE_NAMES = ("ea015_sto_reversal", "ea016_mcp_ddl_cross")
...
def print_creative_summary(
    ...
    print(f"  ({total - zero}/{total} combos produced OOS trades | {beats}/{total} beat benchmark)")
    print()
    ...
    try:
        for idx, params in enumerate(combos, start=1):
            ...
            if idx % 6 == 0 or idx == len(combos):
                ...
                print(
                    f"  [{idx:>2}/{len(combos)}] {elapsed:>6.1f}s "
...
def save_phase4_payload(
    ...
    for strategy_name in CREATIVE_NAMES:
        spec = STRATEGY_SPECS[strategy_name]
        entry = strategy_payload(
            strategy_name,
            spec,
            sorted(creative_rows.get(strategy_name, []), key=row_sort_key, reverse=True),
            creative_total_combos.get(strategy_name, 0),
            "failed" if strategy_name in errors else status,
            errors.get(strategy_name),
        )
        payload["strategies"][spec["label"]] = entry
    save_json(OUT_DIR / "ea_phase4_creative_results.json", payload)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 4 backtest sweep — EA015 + EA016")
    parser.add_argument(
        "--strategies",
        nargs="*",
...
def main() -> int:
    ...
    creative_total_combos: dict[str, int] = {
        name: len(list(itertools.product(*importlib.import_module(name).PARAM_GRID.values())))
    ...
    for strategy_name in selected:
        ...
        save_phase4_payload(
            creative_rows, creative_errors, creative_total_combos,
            "completed" if not creative_errors else "partial",
        )
        print_creative_summary(spec["label"], rows, spec.get("benchmark_pf"))
