"""Phase 3 backtest sweep runner.

Fixes applied vs Phase 2:
  - R012/GJ010: top_n=80 — saves ALL combos, not just top-10
  - EA012: PARAM_GRID calibrated to forex score range; dead trend filter removed
  - EA013: full 96-combo grid (session_start=[0,13] was already in strategy file)
  - EA014: imb_threshold removed (dead on Renko — Elder approx is always 0%/100%)
  - EA012/013/014: benchmarks added (EA008 OOS PF 10.62)

Outputs:
  - ai_context/r012_phase3_results.json
  - ai_context/gj010_phase3_results.json
  - ai_context/ea_phase3_creative_results.json
"""
...
EA_BENCHMARK        = 10.62
EA_BENCHMARK_LABEL  = "EA008 n=5 cd=30"
...
STRATEGY_SPECS: dict[str, dict[str, Any]] = {
    "r012_macd_lc": {
        ...
        "json_path": OUT_DIR / "r012_phase3_results.json",
        "top_n": 80,
        "save_mode": "top",
        "summary": "gate",
    },
    "gj010_macd_lc": {
        ...
        "json_path": OUT_DIR / "gj010_phase3_results.json",
        "top_n": 80,
        "save_mode": "top",
        "summary": "gate",
    },
    "ea012_napoleon_value": {
        ...
        "oos_start": "2025-10-01",
        "oos_end": "2026-03-17",
        "benchmark_pf": EA_BENCHMARK,
        "benchmark_label": EA_BENCHMARK_LABEL,
        "summary": "creative",
    },
...
def top_rows_for_save(spec: dict[str, Any], rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    filtered = sorted(rows, key=row_sort_key, reverse=True)
...
def print_gate_summary(spec: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    ...
    for row in sorted(rows, key=row_sort_key, reverse=True)[:20]:
    ...
    for gate_combo in order:
        ...
        beats = sum(1 for row in subset if row.get("beats_benchmark"))
        print(
            f"{gate_combo:<16} avg OOS PF: {avg_pf:>6.2f}   avg T: {avg_t:>4.0f}   "
            f"beats benchmark: {beats}/{len(subset)}"
        )
    print()


def print_creative_summary(label: str, rows: list[dict[str, Any]], benchmark_pf: float | None) -> None:
    ...
    total = len(rows)
    print(f"  ({total - zero}/{total} combos produced OOS trades)")
    print()
        ...
        for idx, params in enumerate(combos, start=1):
            ...
            if idx % 10 == 0 or idx == len(combos):
                ...
                print(
                    ...
                    f"best OOS PF {format_pf(best['oos_pf'], 6)} "
                    f"({best['oos_trades']}t)"
                )
...
def save_creative_payload(
    ...
    save_json(OUT_DIR / "ea_phase3_creative_results.json", payload)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 3 backtest sweep")
    parser.add_argument(
        "--strategies",
        nargs="*",
        default=[
...
def main() -> int:
    ...
    creative_names = ("ea012_napoleon_value", "ea013_adaptive_escgo", "ea014_alpha_sniper")
    creative_rows: dict[str, list[dict[str, Any]]] = {}
    ...
    for strategy_name in selected:
        ...
        if spec["summary"] == "gate":
            ...
            print_creative_summary(
                spec["label"], rows, spec.get("benchmark_pf")
            )
    ...
    if selected and any(name in creative_rows or name in creative_errors for name in creative_names):
        save_creative_payload(
            creative_rows,
            creative_errors,
            creative_total_combos,
